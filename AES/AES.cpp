...
void AES::ShiftRow(unsigned char state[4][Nb], unsigned int i, unsigned int n)  // shift row i on n positions
{
...
int main(){
  ...
  // encrypt
  std::cout<<"input the text to encrypt: ";
  char data_to_encrypt[2000];
  std::cin.get(data_to_encrypt,2000);
  std::cin.get();
  std::vector<unsigned char> plaintext;
  int count = 0;
  for(; data_to_encrypt[count]!='\0'; count++){
    plaintext.push_back(data_to_encrypt[count]);
  }
  while(count%16!=0){
    plaintext.push_back(' ');
    ++count;
  ...
  std::vector<unsigned char> encrypt_result = aes.EncryptECB(plaintext, key);
  std::cout<< "result ==> ";
  for (int i = 0; i < encrypt_result.size(); i++){
    std::cout<<std::hex;
    if((int)encrypt_result[i]<16){
      std::cout <<'0';
    }
    std::cout << (int)encrypt_result[i];
  }
  std::cout<<std::endl;
  std::cout<<std::endl;

  // decrpyt
  std::cout<<"input the hex to decrypt: ";
  char data_to_decrypt[2000];
  std::cin.get(data_to_decrypt,2000);
  std::vector<unsigned char> ciphertext_to_decrypt;
  for(int i = 0; data_to_decrypt[i]!='\0'; i=i+2){
    int high = data_to_decrypt[i]-48;
    if(high>9){
      high = high-39;
    }
    int low = data_to_decrypt[i+1]-48;
    if(low>9){
      low = low-39;
    }
    int ascii = high*16 + low;
    ciphertext_to_decrypt.push_back((char)ascii);
  }

  std::vector<unsigned char> decrypt_result = aes.DecryptECB(ciphertext_to_decrypt,key);
  std::cout<< "result ==> ";
  for (int i = 0; i < decrypt_result.size(); i++) 
    std::cout << (char)decrypt_result[i];
  std::cout<<std::endl;
}
